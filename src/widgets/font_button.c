...
char *get_font(GtkWidget *font_button) {
    ...
    const gchar *font_name = gtk_font_button_get_font_name(GTK_FONT_BUTTON(font_button));
    return g_strdup(font_name);  // Return a copy that needs to be freed by caller
}

void set_font(GtkWidget *font_button, char *font) {
    /* Set a new font for the font button */
